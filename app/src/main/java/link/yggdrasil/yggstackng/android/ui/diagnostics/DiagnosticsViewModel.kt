package link.yggdrasil.yggstackng.android.ui.diagnostics
...
import link.yggdrasil.yggstackng.android.data.BackupConfig
import link.yggdrasil.yggstackng.android.data.ConfigRepository
import link.yggdrasil.yggstackng.android.data.PortStatsDetail
...
class DiagnosticsViewModel(
    ...
    private val _peerDetails = MutableStateFlow<List<link.yggdrasil.yggstackng.android.data.PeerDetail>>(emptyList())
    val peerDetails: StateFlow<List<link.yggdrasil.yggstackng.android.data.PeerDetail>> = _peerDetails.asStateFlow()
    ...
    private fun parsePeerDetails(json: String): List<link.yggdrasil.yggstackng.android.data.PeerDetail> {
        if (json.isBlank() || json == "[]") return emptyList()
        
        return try {
            val jsonArray = JSONArray(json)
            val peers = mutableListOf<link.yggdrasil.yggstackng.android.data.PeerDetail>()
            
            for (i in 0 until jsonArray.length()) {
                val peerObj = jsonArray.getJSONObject(i)
                val peer = link.yggdrasil.yggstackng.android.data.PeerDetail(
